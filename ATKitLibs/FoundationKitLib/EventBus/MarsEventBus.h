...
///链式配置event的工厂
@interface MarsEventMaker : NSObject

///指定订阅者，指定后当订阅者释放后会自动移除该订阅者的所有订阅，多次调用以最后一次为准，必须参数
@property (nonatomic ,strong ,readonly) MarsEventMaker * (^Target)(id target);

///指定订阅的事件名称，多次调用以最后一次为准，必须参数（若使用联合事件可忽略）
@property (nonatomic ,strong ,readonly) MarsEventMaker *(^EventName)(NSString * eventName);

///指定订阅的副类型，多次调用以最后一次为准，非必须参数
@property (nonatomic ,strong ,readonly) MarsEventMaker *(^SubType)(NSInteger subType);

///指定收到订阅事件后回调的队列，若不指定则在发送队列上回调，多次调用以最后一次为准，非必须参数
@property (nonatomic ,strong ,readonly) MarsEventMaker *(^Queue)(dispatch_queue_t queue);
...
@property (nonatomic ,strong ,readonly) MarsEventMaker *(^UniteEvent)(__kindof MarsEvent * event);
...
@interface MarsEventMaker (AutoBuildWithSelf)
...
///事件总线
@interface MarsEventBus : NSObject
...
-(void)subscribe:(void(^)(MarsEventMaker * maker))makeEvent On:(void(^)(__kindof MarsEvent * event,id subscribeTarget))handleEvent;
///订阅一个普通事件（订阅普通事件需要保证订阅target不为空）
-(void)subscribeEvent:(__kindof MarsEvent *)event target:(id)target On:(void(^)(__kindof MarsEvent * event,id subscribeTarget))handleEvent;

///以工厂发送事件（可以发送普通或联合事件。联合事件发送意为每一个事件都对应发一次，且无先后顺序）
-(void)publish:(void(^)(MarsEventMaker * maker))makeEvent;
///发送一个普通事件
-(void)publishEvent:(__kindof MarsEvent *)event;
...
-(void)remove:(void(^)(MarsEventMaker * maker))makeEvent;
///移除一个普通事件的订阅
