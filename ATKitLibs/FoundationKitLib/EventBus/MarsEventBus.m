...
///实际保存事件的实例
@interface MarsEventEntity : NSObject

@property (nonatomic ,copy) void(^eventHandler)(__kindof MarsEvent * event,id target);
...
-(void)receiveEvent:(__kindof MarsEvent *)event target:(id)target;

-(void)resetUniteCondition;

@end

@implementation MarsEventEntity
...
-(void)doEventHanlerWithEvent:(MarsEvent *)event target:(id)target {
    if (self.queue) {
...
-(void)setUniteEvents:(NSMutableSet *)uniteEvents {
    ...
    [uniteEvents enumerateObjectsUsingBlock:^(__kindof MarsEvent * obj, BOOL * _Nonnull stop) {
        @autoreleasepool {
...
@interface MarsEventProxy : NSProxy
...
@implementation MarsEventProxy

+(instancetype)proxyWithTarget:(id)target {
    MarsEventProxy * p = [MarsEventProxy alloc];
    p.target = target;
    return p;
}
...
///对象管理所有事件的中间订阅者
@interface MarsEventSubscriber : NSObject

@property (nonatomic ,weak) id target;

@property (nonatomic ,strong) MarsEventProxy * proxy;

@property (nonatomic ,strong) NSMutableDictionary * eventsMap;

@property (nonatomic ,weak) MarsEventBus * bus;

@property (nonatomic ,strong) dispatch_semaphore_t sema;

+(instancetype)subscriberWithTaget:(id)target bus:(MarsEventBus *)bus;

-(void)receiveEvent:(__kindof MarsEvent *)event;

@end

@implementation MarsEventSubscriber

#pragma mark --- interface method ---
///非线程安全，应由外界保证线程安全
+(instancetype)subscriberWithTaget:(id)target bus:(MarsEventBus *)bus {
    MarsEventSubscriber * sub = objc_getAssociatedObject(target, [bus.uid UTF8String]);
    if (!sub) {
        sub = [MarsEventSubscriber new];
        sub.target = target;
        sub.bus = bus;
        sub.proxy = [MarsEventProxy proxyWithTarget:sub];
...
-(void)receiveEvent:(__kindof MarsEvent *)event {
    ...
    [entitys enumerateObjectsUsingBlock:^(MarsEventEntity * _Nonnull obj, BOOL * _Nonnull stop) {
...
@property (nonatomic ,strong) NSMutableSet <__kindof MarsEvent *>* _uniteEvents;

@property (nonatomic ,strong) NSMutableSet <__kindof MarsEvent *>* events;

@end

@implementation MarsEventMaker
...
#pragma mark --- setter/getter ---
-(MarsEventMaker *(^)(id))Target {
    return ^MarsEventMaker *(id target) {
        self._target = target;
        return self;
    };
}

-(MarsEventMaker *(^)(NSString *))EventName {
    return ^MarsEventMaker *(NSString * eventName) {
        self._eventName = eventName;
        return self;
    };
}

-(MarsEventMaker *(^)(NSInteger))SubType {
    return ^MarsEventMaker *(NSInteger subType) {
        if (subType >= 0) {
...
-(MarsEventMaker *(^)(dispatch_queue_t))Queue {
    return ^MarsEventMaker *(dispatch_queue_t queue) {
        self._queue = queue;
        return self;
    };
}

-(MarsEventMaker *(^)(__kindof MarsEvent *))UniteEvent {
    return ^MarsEventMaker *(__kindof MarsEvent * event) {
        if ([event valid]) {
...
-(NSMutableSet <__kindof MarsEvent *>*)_uniteEvents {
    if (!__uniteEvents) {
...
-(NSMutableSet <__kindof MarsEvent *>*)events {
    if (!_events) {
...
@implementation MarsEventMaker (AutoBuildWithSelf)
...
@implementation MarsEventBus

#pragma mark --- interface method ---

+(instancetype)defaultEventBus {
    static MarsEventBus * defaultBus = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        defaultBus = [[MarsEventBus alloc] init];
    });
    return defaultBus;
}

-(void)subscribe:(void (^)(MarsEventMaker *))makeEvent On:(void (^)(__kindof MarsEvent * ,id))handleEvent {
    ///没有事件或者没有回调均不作处理
    if (!makeEvent || !handleEvent) {
        return;
    }
    MarsEventMaker * maker = [MarsEventMaker new];
    makeEvent(maker);
    ///如果没有有效事件不作处理
     ...
     */
    dispatch_semaphore_wait(self.sema, DISPATCH_TIME_FOREVER);
    [maker.events enumerateObjectsUsingBlock:^(MarsEvent * obj, BOOL * _Nonnull stop) {

        ///取出观察者
        MarsEventSubscriber * sub = [MarsEventSubscriber subscriberWithTaget:obj.target bus:self];
        ///创建实例
        MarsEventEntity * entity = [MarsEventEntity new];
        ...
        if (obj.uniteEvents.count) {
            ...
            [obj.uniteEvents enumerateObjectsUsingBlock:^(MarsEvent * subObj, BOOL * _Nonnull stop) {
                ///将实例分别注册到每一个事件中
...
-(void)subscribeEvent:(__kindof MarsEvent *)event target:(id)target On:(void (^)(__kindof MarsEvent *, id))handleEvent {
    if (!target || ![event valid]) {
        return;
    }
    dispatch_semaphore_wait(self.sema, DISPATCH_TIME_FOREVER);
    MarsEventSubscriber * sub = [MarsEventSubscriber subscriberWithTaget:target bus:self];
    MarsEventEntity * entity = [MarsEventEntity new];
    entity.eventHandler = handleEvent;
...
-(void)publish:(void (^)(MarsEventMaker *))makeEvent {
    if (!makeEvent) {
        return;
    }
    MarsEventMaker * maker = [MarsEventMaker new];
    ...
    [maker.events enumerateObjectsUsingBlock:^(__kindof MarsEvent * _Nonnull obj, BOOL * _Nonnull stop) {
        if (obj.uniteEvents.count) {
            [obj.uniteEvents enumerateObjectsUsingBlock:^(__kindof MarsEvent * subObj, BOOL * _Nonnull stop) {
...
-(void)publishEvent:(MarsEvent *)event {
    ///没有事件类型不做操作
    if (![event valid]) {
        return;
    }
    ...
    ///派发事件
    dispatch_semaphore_wait(self.sema, DISPATCH_TIME_FOREVER);
    NSSet <MarsEventSubscriber *>* subs = [self.subscribersMap valueForKey:event.eventName];
    [subs enumerateObjectsUsingBlock:^(MarsEventSubscriber * _Nonnull obj, BOOL * _Nonnull stop) {
        if (event.queue) {
...
-(void)remove:(void (^)(MarsEventMaker *))makeEvent {
    ///没有生成s事件则不做操作
    if (!makeEvent) {
        return;
    }
    ...
    dispatch_semaphore_wait(self.sema, DISPATCH_TIME_FOREVER);
    [maker.events enumerateObjectsUsingBlock:^(__kindof MarsEvent * obj, BOOL * _Nonnull stop) {
        ///取出订阅者
        MarsEventSubscriber * sub = [MarsEventSubscriber subscriberWithTaget:obj.target bus:self];
        if (obj.uniteEvents.count) {
            [obj.uniteEvents enumerateObjectsUsingBlock:^(__kindof MarsEvent * subObj, BOOL * _Nonnull stop) {
...
-(void)removeEvent:(__kindof MarsEvent *)event target:(id)target {
    if (!target || ![event valid]) {
        return;
    }
    dispatch_semaphore_wait(self.sema, DISPATCH_TIME_FOREVER);
...
#pragma mark --- tool method ---
-(void)registEvent:(__kindof MarsEvent *)event onSubscriber:(MarsEventSubscriber *)sub entity:(MarsEventEntity *)entity {
    ///在bus上注册
...
-(void)deregistEvent:(__kindof MarsEvent *)event onSubscriber:(MarsEventSubscriber *)sub {
    ...
    [entitys enumerateObjectsUsingBlock:^(MarsEventEntity * obj, BOOL * _Nonnull stop) {
        ///存在即为联合事件
